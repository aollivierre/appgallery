...
$Schedule_StartDate = "2023-11-10"  # YYYY.MM.DD        (for Once)
$Schedule_StartTime = "1pm"         # ex 8am or 5pm     (for Once, Hourly, Daily)
...
try{
    ...
    if($task_existing.Description -like "Version $Version*"){
        ...
        Copy-Item "$d\detection.ps1"-Destination $Path_PR -Force
        Copy-Item "$d\remediation.ps1" -Destination $Path_PR -Force
