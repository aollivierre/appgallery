...
$LogName = (Get-Date -Format "HHmmss") + "_BackupOutlookSignatures"
$EventSource = (Get-Date -Format "HHmmss") + "_BackupOutlookSignatures"
...
# Write-CustomEventLog -LogName $LogName -EventSource $EventSource -EventMessage "Outlook Signature Backup completed with warnings." -EventID 1001 -Level 'WARNING'
...
try {
    ...
    if ($allScanResults.Count -gt 0) {
        ...
        $messageText = "Computer $computerName signatures is now backed up - review logs uploaded on SharePoint online 'Syslog' for more details"
