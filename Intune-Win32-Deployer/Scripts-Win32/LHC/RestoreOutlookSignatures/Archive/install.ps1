...
#     param (
#         [string]$LogName = 'RestoreOutlookSignaturesLog',
#         [string]$source = (Get-PSCallStack)[1].Command
#         # $source
#     )

#     if ([string]::IsNullOrEmpty($LogName) -or [string]::IsNullOrEmpty($source)) {
#         Write-Host "Both LogName and source must have valid non-empty values." -ForegroundColor Red
#         return
#     }
...
#                 Write-Host "Attempting to create event source '$source' in log '$LogName'..." -ForegroundColor Yellow
...
#         }
#     } catch {
#         Write-Host "Error creating event source '$source' in log '$LogName': $($_.Exception.Message)" -ForegroundColor Red
#     }
# }
...
# CreateEventLogSource -source $source -LogName $loganme
...
#     Write-Host "Creating event log source: $source"
...
#             Write-Host "Writing event log entry (PSVersion < 6) with LogName: $logName, Source: $source, EntryType: $entryType, EventId: $eventID, Message: $Message"
...
# Call the Write-CustomEventLog function with custom parameters
# Write-CustomEventLog -EventMessage "Outlook Signature Restore completed successfully." -EntryType Information

# Call the Write-CustomEventLog function with custom parameters and a different EventID
# Write-CustomEventLog -EventMessage "Outlook Signature Restore failed." -EventID 1001 -EntryType Error
...
#     switch ($EntryType) {
#         'INFORMATION'    { $ForegroundColor = [ConsoleColor]::Green }
...
#     Write-CustomEventLog -EventMessage $formattedMessage -EntryType $EntryType
# }
...
function CreateDetectionFile {
    $ErrorActionPreference = 'SilentlyContinue'
    $filePath = "C:\Intune\Win32\RestoreOutlookSignatures\detect$file_extension"
    $hadError = $false
    ...
    if (-not $hadError) {
        Write-EnhancedLog -Message "Detection file creation completed successfully. file C:\Intune\Win32\RestoreOutlookSignatures\detect$file_extension " -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)
    }
}
