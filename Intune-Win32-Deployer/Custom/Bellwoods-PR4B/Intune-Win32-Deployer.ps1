...
# $CustomWin32AppName = "APP:001 - Microsoft 365 Apps for Business - No Access - No Publisher"
# $CustomWin32AppName = "Windows Package Manager"



$connectionParams = @{
    clientId = "xxxxxxxx-4eb1d0b50e92"
    tenantID = "xxxxxxxxxxx-c15d21a89f13"
    ClientSecret = "xxxxxxxxxxxxxxxxxxxxxxxx"  # Replace with your actual Client Secret
}
...
#     # Ensure $connectionParams and $TenantName are defined and accessible in this scope
#     param (
#         [Parameter(Mandatory)]
...
#         Client_Id     = $connectionParams.clientId  
#         Client_Secret = $connectionParams.clientSecret
#     }  

#     try {
#         $tokenResponse = Invoke-RestMethod -Uri "https://login.microsoftonline.com/$TenantName/oauth2/v2.0/token" -Method POST -Body $tokenBody -ContentType "application/x-www-form-urlencoded" -ErrorAction Stop
...
# Usage example (ensure you define $connectionParams and $TenantName before calling the function)
# $connectionParams = @{
...
# Write-EnhancedLog -Message "Calling Get-MicrosoftGraphAccessToken to obtain access token" -Level "WARNING" -ForegroundColor ([ConsoleColor]::Yellow)

# try {
#     $global:accessTokenfromgraph = Get-MicrosoftGraphAccessToken -connectionParams $connectionParams -TenantName $TenantName

#     if ($null -ne $global:accessTokenfromgraph) {
#         Write-EnhancedLog -Message "Successfully obtained access token using Get-MicrosoftGraphAccessToken. $global:accessTokenfromgraph" -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)
#     } else {
#         Write-EnhancedLog -Message "Failed to obtain access token using Get-MicrosoftGraphAccessToken. The returned access token is null." -Level "ERROR" -ForegroundColor ([ConsoleColor]::Red)
#     }
# } catch {
#     Write-EnhancedLog -Message "An error occurred while calling Get-MicrosoftGraphAccessToken to obtain an access token: $_" -Level "ERROR" -ForegroundColor ([ConsoleColor]::Red)
# }
    ...
    process {
        ...
        if ([string]::IsNullOrWhiteSpace($userInput)) {
            if ([string]::IsNullOrWhiteSpace($global:AOguidString)) {
                ...
                Get-CustomWin32AppName # Recursively call the function until valid input is provided or global GUID is set.
            } else {
...
function Create-WingetWin32App($Prg) {
    ...
    Write-Host "Create win32 package for $global:CustomWin32AppName" -Foregroundcolor cyan
...
function Upload-Win32App ($Prg, $Prg_Path, $Prg_img) {
    ...
    try {
        ...
        # $refreshToken = '0.Axxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'
    ...
    Write-EnhancedLog -Message "calling Create-AADGroup" -Level "WARNING" -ForegroundColor ([ConsoleColor]::Yellow)
    if ($($global:SettingsVAR.AADgrp) -eq "True") { Create-AADGroup $Prg }
    Write-EnhancedLog -Message "calling Create-AADGroup - done" -Level "INFO" -ForegroundColor ([ConsoleColor]::GREEN)
...
function Import-FromCatalog {
    ...
    Write-EnhancedLog -Message "calling Read-AppRepo" -Level "WARNING" -ForegroundColor ([ConsoleColor]::Yellow)
    $Prg_selection = Read-AppRepo
...
function Create-AADGroup ($Prg) {
    ...
    # if ($global:accessTokenfromgraph -isnot [string]) {
    #     Write-EnhancedLog -Message "The access token is not a string. Check the Get-MicrosoftGraphAccessToken function." -Level "ERROR" -ForegroundColor ([ConsoleColor]::Red)
    #     return
    # }
    
    # Write-EnhancedLog -Message "Calling Connect-MgGraph with Access token $global:accessToken" -Level "WARNING" -ForegroundColor ([ConsoleColor]::Yellow)
    
    # # Proceed to connect to MgGraph
    # $MGSession = Connect-MgGraph -AccessToken $global:accessToken -Scopes $global:scopes
    


    # Write-EnhancedLog -Message "calling Connect-MgGraph with Access token $global:accessTokenfromgraph " -Level "WARNING" -ForegroundColor ([ConsoleColor]::Yellow)

    
    # Connect AAD if not connected
    # $MGSession = Connect-MgGraph -AccessToken $global:accessTokenfromgraph -Scopes $global:scopes
    # $MGSession = Connect-MgGraph -Scopes $global:scopes
...
# $TenantId = "xxxxxxxxxxxxx-c15d21a89f13"
...
# $connectionParams = @{
#     clientId = "xxxxxxxxxxxxxxxx-4eb1d0b50e92"
#     tenantID = "xxxxxxxxxxxxxxxxx-c15d21a89f13"
#     ClientSecret = "xxxxxxxxxxxxxxxxxxx"  # Replace with your actual Client Secret
# }
# $TenantName = "bcclsp.org"
