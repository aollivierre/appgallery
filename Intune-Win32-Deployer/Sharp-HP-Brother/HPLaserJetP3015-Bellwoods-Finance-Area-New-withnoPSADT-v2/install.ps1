...
#     Write-Host "Installation script started."
...
#     # Check if the INF file exists before proceeding with driver installation
#     if (Test-Path -Path $InfPath) {
#         Write-Host "INF file found. Proceeding with driver installation..."
...
#             }
#             Catch {
#                 Write-Host "Failed to install printer driver. Error: $_"
#             }
#         }
#         else {
#             # Write-Host "Printer driver already exists. Skipping installation."
#         }
#     }
#     else {
#         # Write-Host "INF file does not exist at the specified path: $InfPath"
#     }
...
#         # Write-Host "Printer port added."
#         Start-Sleep 120
#     }
#     else {
#         Write-Host "Printer port already exists."
#     }
...
#         Add-Printer -Name $PrinterName -DriverName $DriverName -PortName $PortName
#         # Write-Host "Existing printer removed and new printer added."
#         Start-Sleep 120
#     }
...
# $DriverName = "SHARP MX-3071 PCL6"
# $InfPathRelativeToscriptDirectory = "Driver\su2emenu.inf"
# $CertificatePathRelativeToscriptDirectory = "sharp.cer"
...
#     Write-EnhancedLog -Message "Installation script started." -Level "INFO" -ForegroundColor Green
#     Write-EnhancedLog -Message "INF Path: $InfPath" -Level "VERBOSE" -ForegroundColor Cyan
#     Write-EnhancedLog -Message "Certificate Path: $CertPath" -Level "VERBOSE" -ForegroundColor Cyan
...
#             Import-Certificate -FilePath $CertPath -CertStoreLocation Cert:\LocalMachine\TrustedPublisher
#             Import-Certificate -FilePath $CertPath -CertStoreLocation Cert:\CurrentUser\TrustedPublisher
#             Write-EnhancedLog -Message "Certificate imported successfully." -Level "INFO" -ForegroundColor Green
#         }
#         Catch {
...
#     # Check if the INF file exists before proceeding with driver installation
#     if (Test-Path -Path $InfPath) {
#         Write-EnhancedLog -Message "INF file found. Proceeding with driver installation..." -Level "INFO" -ForegroundColor Green
#         # Check if the printer driver exists
#         $DriverExists = Get-PrinterDriver -Name $DriverName -ErrorAction SilentlyContinue
#         if (-not $DriverExists) {
#             Try {
#                 pnputil.exe /add-driver $InfPath /install
#                 Add-PrinterDriver -Name $DriverName
#                 Write-EnhancedLog -Message "Printer driver installed successfully." -Level "INFO" -ForegroundColor Green
#                 Start-Sleep -Seconds 120
#             }
#             Catch {
#                 Write-EnhancedLog -Message "Failed to install printer driver. Error: $_" -Level "ERROR" -ForegroundColor Red
#             }
#         }
#         else {
#             Write-EnhancedLog -Message "Printer driver already exists. Skipping installation." -Level "INFO" -ForegroundColor Yellow
#         }
#     }
#     else {
...
#     if (-not $PortExists) {
#         Add-PrinterPort -Name $PortName -PrinterHostAddress $PrinterIPAddress
#         Write-EnhancedLog -Message "Printer port added." -Level "INFO" -ForegroundColor Green
#         Start-Sleep -Seconds 120
#     }
#     else {
#         Write-EnhancedLog -Message "Printer port already exists." -Level "INFO" -ForegroundColor Yellow
#     }
...
#         Add-Printer -Name $PrinterName -DriverName $DriverName -PortName $PortName
#         Start-Sleep -Seconds 120
#         Write-EnhancedLog -Message "Printer added." -Level "INFO" -ForegroundColor Green
#     }
#     else {
#         Remove-Printer -Name $PrinterName
#         Add-Printer -Name $PrinterName -DriverName $DriverName -PortName $PortName
#         Write-EnhancedLog -Message "Existing printer removed and new printer added." -Level "INFO" -ForegroundColor Green
#         Start-Sleep -Seconds 120
#     }

#     Write-EnhancedLog -Message "Installation script completed." -Level "INFO" -ForegroundColor Green
# }
...
# # Calling Initialize-PrinterInstallationVariables to set up our environment
# $initializationInfo = Initialize-PrinterInstallationVariables -InfPathRelativeToscriptDirectory "Driver\su2emenu.inf" -CertificatePathRelativeToscriptDirectory "sharp.cer" -PrinterIPAddress "192.168.1.100" -ScriptDirectory $scriptDirectory

# # Extracting printer installation-specific variables from the initialization output
# $InfPath = $initializationInfo.InfPath
# $CertPath = $initializationInfo.CertPath
# $PortName = $initializationInfo.PortName
...
# Define the base script directory and parameters for printer installation
# $scriptDirectory = Split-Path -Parent -Path $MyInvocation.MyCommand.Definition
# $PrinterName = "SHARP MX-3071"
# $PrinterIPAddress = "192.168.1.100"
# $DriverName = "SHARP MX-3071 PCL6"
# # $InfPathRelative = "Driver\su2emenu.inf"
# $InfPathRelative = "Driver\hpcu270u.inf"
# # $CertificatePathRelative = "sharp.cer"
