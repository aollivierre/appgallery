...
Try {
    ...
    ## Variables: Script
    [String]$deployAppScriptFriendlyName = 'Deploy FortiClient EMS V7.0.9'
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        $specifiedProductCode = "{D6A52B20-063A-4BF6-8228-CDADBF8ACBCF}"
        $uninstallString = $null
        $foundProductCode = $null

        foreach ($key in $uninstallKeys) {
            Get-ChildItem -Path $key |
            ForEach-Object {
                $app = Get-ItemProperty -Path $_.PsPath
                if ($app.DisplayName -like "*Forti*" -and $app.PSChildName -ne $specifiedProductCode) {
                    $uninstallString = $app.UninstallString
                    $foundProductCode = $app.PSChildName
                    break
                }
            }
        ...
        if ($null -ne $uninstallString) {
            Write-Host "Found uninstall string for product code $foundProductCode - $uninstallString"
            # Extract the MSI product code from the uninstall string
            $uninstallProductCode = $uninstallString -replace ".*\{(.*)\}.*", '{$1}'
            # Construct the MSIEXEC command with silent uninstall
            ...
            Write-Host "Uninstall string not found for any other version of FortiClient."
        }
