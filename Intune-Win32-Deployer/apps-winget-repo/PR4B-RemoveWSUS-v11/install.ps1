...
$initializationInfoScript = Initialize-ScriptVariables @InitializeScriptVariablesParams

$initializationInfoScript

$global:PackageName = $initializationInfoScript['PackageName']
$global:PackageUniqueGUID = $initializationInfoScript['PackageUniqueGUID']
$global:Version = $initializationInfoScript['Version']
$global:ScriptMode = $initializationInfoScript['ScriptMode']
$global:Path_local = $initializationInfoScript['Path_local']
$global:Path_PR = $initializationInfoScript['Path_PR']
$global:schtaskName = $initializationInfoScript['schtaskName']
$global:schtaskDescription = $initializationInfoScript['schtaskDescription']
...
function Copy-FilesToPath {
    ...
    try {
        ...
        # Ensure DestinationPath is not the same as $PSScriptRoot
        if ($PSScriptRoot -eq $DestinationPath) {
            Write-EnhancedLog -Message "Source and destination paths are the same. Copy operation skipped." -Level "WARNING" -ForegroundColor ([System.ConsoleColor]::Yellow)
            return
        }
        ...
        foreach ($File in $Files) {
            ...
            if (Test-Path -Path $destinationFile) {
                Write-EnhancedLog -Message "File $File.Name already exists at destination. Overwriting due to -Force flag." -Level "INFO" -ForegroundColor ([System.ConsoleColor]::DarkYellow)
            }
...
function Execute-DetectionAndRemediation {
    ...
    try {
        Write-EnhancedLog -Message "Executing detection and remediation scripts in $PSScriptRoot..." -Level "INFO" -ForegroundColor Magenta

        # Execution of the detection script directly using $PSScriptRoot
...
function SetupNewTaskEnvironment {
    ...
    param (
        ...
        [Parameter(Mandatory = $true)]
        [ValidateSet("Remediation", "DetectAndRemediate")]
        [string]$ScriptMode
    )
    ...
    try {
        ...
        $Path_PSscript = switch ($ScriptMode) {
            ...
            "DetectAndRemediate" { Join-Path $Path_PR "install.ps1" }
            Default { throw "Invalid ScriptMode: $ScriptMode. Expected 'Remediation' or 'DetectAndRemediate'." }
        }
