function Copy-SetupAndXMLToMEMFolder {
    # Define the target directory
    $targetDir = "C:\_MEM\Scripts"
    ...
    # Define the paths of the setup.exe and current.xml in the current script's directory
    $sourceSetup = Join-Path -Path $scriptDirectory -ChildPath "setup.exe"
    $sourceXML = Join-Path -Path $scriptDirectory -ChildPath "current.xml"

    # Check if the setup.exe exists in the current directory and copy it
    if (Test-Path $sourceSetup) {
        Copy-Item -Path $sourceSetup -Destination $targetDir -Force
        # Write-Output "Copied setup.exe to $targetDir"
    } else {
        # Write-Error "setup.exe does not exist in the current directory."
    }

    # Check if the current.xml exists in the current directory and copy it
    if (Test-Path $sourceXML) {
        Copy-Item -Path $sourceXML -Destination $targetDir -Force
        # Write-Output "Copied current.xml to $targetDir"
    } else {
        # Write-Error "current.xml does not exist in the current directory."
    }
}

# Call the function
Copy-SetupAndXMLToMEMFolder



$PackageName = "PR4B_UpdateM365CurrentChannel"
$Version = 1
...
$Schedule_StartDate = "2023-10-13"  # YYYY.MM.DD        (for Once)
