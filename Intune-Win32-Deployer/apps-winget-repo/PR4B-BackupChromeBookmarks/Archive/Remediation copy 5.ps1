...
foreach ($file in $filesToBackup) {
    ...
    if (Test-Path -Path (Join-Path -Path $sourceFilePath -ChildPath $file)) {
        try {
            # Robocopy parameters to copy specific files, retry options
            # Suppressing Robocopy output by redirecting to null
            $robocopyParams = @($sourceFilePath, $destinationFilePath, $file, "/COPY:DAT", "/R:0", "/W:0", "/NP") > $null
            robocopy @robocopyParams
            
            # Check the exit code ($LASTEXITCODE) for success (0, 1)
