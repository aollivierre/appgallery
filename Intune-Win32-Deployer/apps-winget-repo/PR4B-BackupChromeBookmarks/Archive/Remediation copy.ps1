...
function Copy-ItemsWithRobocopy {
    ...
    # Ensure the destination directory 'ChromeBackup' exists
    $finalDestinationPath = Join-Path -Path $DestinationPath -ChildPath "ChromeBackup"
    ...
    $robocopyArgs = @("${SourcePath}\", $finalDestinationPath, "/E", "/R:0", "/W:0", "/V", "/LOG:robocopy.log")
    ...
    try {
        ...
        switch ($LASTEXITCODE) {
            ...
            16 { Write-Error "Robocopy failed: Serious error. Robocopy did not copy any files. This is either a usage error or an error due to insufficient access privileges on the source or destination directories." }
...
$ChromesourcePath = "$env:USERPROFILE\AppData\Local\Google\Chrome\User Data"
$ChromebackupDirectory = "ChromeBackup" # Modify as needed if you want the backup in a different location
$ChromebackupPath = Join-Path -Path $oneDriveDirectory -ChildPath $ChromebackupDirectory

# Check if the source directory exists
if (Test-Path -Path $ChromesourcePath) {
    # Ensure the destination directory exists; if not, create it
    if (-not (Test-Path -Path $ChromebackupPath)) {
        New-Item -Path $ChromebackupPath -ItemType Directory
    }

    # Use splatting for function parameters
    $params = @{
        SourcePath      = $ChromesourcePath
        DestinationPath = $ChromebackupPath
        # Add additional Robocopy options as needed. Example: "/E" for copying subdirectories, including empty ones.
        # Options         = "/E" # Modify or extend the options as required for your backup needs
    }

    # Assuming 'Copy-ItemsWithRobocopy' is a custom function you have that wraps around Robocopy,
