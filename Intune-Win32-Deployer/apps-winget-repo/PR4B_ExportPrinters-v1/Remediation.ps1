...
#         $file_path, 
#         $folder_name,
#         $accessToken # Ensure you're passing this parameter correctly
#     )

#     # Use the -Raw parameter to preserve the file content formatting
...
#     # $upload_headers = @{
#     #     "Authorization" = "Bearer $accessToken"
#     #     "Content-Type"  = "text/plain; charset=utf-8" # Ensure correct character encoding
#     # }
...
# <#
# .SYNOPSIS
# Zips a specified file.

# .DESCRIPTION
# This function takes a file path as input and compresses it into a zip file in the same directory. It logs the process using a custom logging function, Write-EnhancedLog.

# .PARAMETER FilePath
# The path of the file to be zipped.

# .PARAMETER DestinationPath
# The destination path of the zipped file. If not provided, the zip file is created in the same directory as the original file with the same name plus '.zip' extension.

# .EXAMPLE
# Zip-File -FilePath "C:\path\to\your\file.txt"

# Compresses file.txt into file.txt.zip in the same directory.
# #>
...
#         $DestinationPath = "$FilePath.zip"
#     }

#     Write-EnhancedLog -Message "Starting to zip file: $FilePath" -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)
    
#     try {
#         Compress-Archive -Path $FilePath -DestinationPath $DestinationPath -Force
#         Write-EnhancedLog -Message "Successfully zipped file to: $DestinationPath" -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)
#     }
#     catch {
#         Write-EnhancedLog -Message "Failed to zip file: $_" -Level "ERROR" -ForegroundColor ([ConsoleColor]::Red)
#         throw $_
#     }
# }
...
# Zip-File -FilePath $filePath # DestinationPath is optional

# # Now, upload the zipped file
# $zippedFilePath = "$filePath.zip" # Assuming the zip file is in the same directory
# Upload-FileToSharePoint -document_drive_id "your_drive_id" -file_path $zippedFilePath -folder_name "your_folder_name" -accessToken "your_access_token"
...
# $exportsFolderPath = "$PSScriptRoot\exports"
...
# Upload the zipped exports folder to SharePoint
# Upload-FileToSharePoint -document_drive_id "your_drive_id" -file_path $zippedExportsPath -folder_name "your_folder_name" -accessToken "your_access_token"
...
#     # Zip the file
#     # Compress-Archive -Path $file_path -DestinationPath $zipFilePath -Force

#     # Read the zip file content as a byte array
#     $fileContent = [System.IO.File]::ReadAllBytes($zipFilePath)
#     $filename = (Get-Item -Path $zipFilePath).Name
...
#     $uploadResponse = Invoke-RestMethod -Headers $upload_headers -Uri $puturl -Method PUT -Body $fileContent -ContentType "application/octet-stream"
    
#     # Optionally, delete the zip file after upload to clean up
...
#     try {
#         # Enhanced logging for start
#         Write-EnhancedLog -Message "Starting file upload process to SharePoint" -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)
...
#             Write-EnhancedLog -Message "Zipping the file at path: $file_path" -Level "VERBOSE" -ForegroundColor ([ConsoleColor]::Cyan)
#             Compress-Archive -Path $file_path -DestinationPath $zipFilePath -Force
#             Write-EnhancedLog -Message "File zipped successfully: $zipFilePath" -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)
#         } else {
#             Write-EnhancedLog -Message "Zip file already exists. Proceeding with upload: $zipFilePath" -Level "WARNING" -ForegroundColor ([ConsoleColor]::Yellow)
#         }
...
#         Write-EnhancedLog -Message "Uploading file to SharePoint" -Level "VERBOSE" -ForegroundColor ([ConsoleColor]::Cyan)
#         $uploadResponse = Invoke-RestMethod -Headers $upload_headers -Uri $puturl -Method PUT -Body $fileContent -ContentType "application/octet-stream"
        
#         # Success logging
#         Write-EnhancedLog -Message "File uploaded successfully to SharePoint" -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)

#         # Optionally, delete the zip file after upload to clean up
#         Write-EnhancedLog -Message "Cleaning up: Deleting zip file $zipFilePath" -Level "VERBOSE" -ForegroundColor ([ConsoleColor]::Cyan)
#         Remove-Item -Path $zipFilePath
#         Write-EnhancedLog -Message "Cleanup completed" -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)

#     } catch {
#         Write-EnhancedLog -Message "An error occurred during the file upload process: $_" -Level "ERROR" -ForegroundColor ([ConsoleColor]::Red)
#         throw $_
#     }
# }
...
#         # Performing the file upload
#         Write-EnhancedLog -Message "Uploading zipped folder to SharePoint" -Level "VERBOSE" -ForegroundColor ([ConsoleColor]::Cyan)
...
#         # Success logging
#         Write-EnhancedLog -Message "Zipped folder uploaded successfully to SharePoint." -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)
