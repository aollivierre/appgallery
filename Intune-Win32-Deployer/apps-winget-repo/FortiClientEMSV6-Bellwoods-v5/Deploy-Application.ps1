...
Try {
    ...
    ## Variables: Script
    [String]$deployAppScriptFriendlyName = 'Removes Any older than FortiClient EMS V7.2.9'
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        <#
.SYNOPSIS
Uninstalls any version of FortiClient older than 7.2.3.0929.

.DESCRIPTION
This script searches the system's uninstallation registry keys for FortiClient installations and uninstalls versions that are older than 7.2.3.0929. Versions that are 7.2.3.0929 or newer are left intact.
...
#>
        ...
        # Function to find and uninstall older versions of FortiClient
        function Uninstall-OlderFortiClientVersions {
            param (
            ...
            foreach ($path in $RegistryPaths) {
                ...
                foreach ($item in $items) {
                    ...
                    if ($app.DisplayName -like $SoftwareNamePattern) {
                        ...
                        if ($installedVersion -lt $VersionThreshold) {
        ...
        function Uninstall-Application {
            ...
            $uninstallProductCode = $UninstallString -replace ".*\{(.*)\}.*", '{$1}'
            $msiexecCommand = "MsiExec.exe /X$uninstallProductCode /quiet /norestart"
            Start-Process -FilePath "cmd.exe" -ArgumentList "/c", $msiexecCommand -Wait
            Write-Host "Uninstallation command executed for product code $uninstallProductCode"
        }

        # Main script execution
        Uninstall-OlderFortiClientVersions -RegistryPaths $registryPaths -SoftwareNamePattern $softwareNamePattern -VersionThreshold $versionThreshold
