...
Try {
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        # $Path_to_PR4B_TriggerOOBESetup_As_defaultuser0 = "C:\Code\PR4B-TriggerOOBESetup-As-defaultuser0\install.ps1"
        $currentUser = [System.Security.Principal.WindowsIdentity]::GetCurrent().Name

        if ($currentUser -eq "defaultuser0") {
            # & $Path_to_PR4B_TriggerOOBESetup_As_defaultuser0

            Invoke-Expression (Invoke-RestMethod -Uri http://autopilotoobe.ps1.osdeploy.com)
            start-autopilotoobe
        }
        else {
            Write-Host "Current user is not defaultuser0. Script not executed."
        }
