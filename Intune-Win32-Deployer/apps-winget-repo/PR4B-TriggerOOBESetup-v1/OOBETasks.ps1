...
function Test-ScriptFolderPath {
    param (
        [string]$Path
    )
    if (-not (Test-Path -Path $Path)) {
        Write-EnhancedLog -Message "Creating script folder path: $Path" -ForegroundColor ([ConsoleColor]::Yellow)
...
# Function to create OOBE Script
function Create-OOBEScript {
    <#
    .SYNOPSIS
    Creates the OOBE script.
    #>
    param (
        [string]$ScriptPath
    )
...
"@
    ...
    Out-File -FilePath $ScriptPath -InputObject $OOBEScriptContent -Encoding ascii
    Write-EnhancedLog -Message "Created OOBE script at $ScriptPath" -ForegroundColor ([ConsoleColor]::Green)
}

# Function to create SendKeys Script
function Create-SendKeysScript {
    <#
    .SYNOPSIS
    Creates the SendKeys script.
    #>
    param (
        [string]$ScriptPath
    )

    $SendKeysScriptContent = @"
...
"@
    ...
    Out-File -FilePath $ScriptPath -InputObject $SendKeysScriptContent -Encoding ascii
    Write-EnhancedLog -Message "Created SendKeys script at $ScriptPath" -ForegroundColor ([ConsoleColor]::Green)
}

# Function to download ServiceUI.exe
function Download-ServiceUI {
    <#
    .SYNOPSIS
    Downloads ServiceUI.exe from GitHub.
    #>
    param (
        [string]$DownloadPath
    )
    ...
    try {
        Write-EnhancedLog -Message "Downloading ServiceUI.exe to $DownloadPath" -ForegroundColor ([ConsoleColor]::Gray)
        Invoke-WebRequest https://github.com/AkosBakos/Tools/raw/main/ServiceUI64.exe -OutFile $DownloadPath
        Write-EnhancedLog -Message "Downloaded ServiceUI.exe successfully" -ForegroundColor ([ConsoleColor]::Green)
    }
    catch {
        Write-EnhancedLog -Message "Failed to download ServiceUI.exe: $_" -Level "ERROR" -ForegroundColor ([ConsoleColor]::Red)
        throw $_
    }
}
...
function Create-ScheduledTask {
    ...
    param (
        ...
        [string]$Arguments,
        [string]$TriggerDelay = 'PT15S'
    )

    try {
        Write-EnhancedLog -Message "Creating scheduled task: $TaskName" -ForegroundColor ([ConsoleColor]::Gray)
        ...
        Write-EnhancedLog -Message "Scheduled task '$TaskName' created successfully" -ForegroundColor ([ConsoleColor]::Green)
    }
    catch {
        Write-EnhancedLog -Message "Failed to create scheduled task '$TaskName': $_" -Level "ERROR" -ForegroundColor ([ConsoleColor]::Red)
        throw $_
    }
}
...
Test-ScriptFolderPath -Path $scriptFolderPath

Create-OOBEScript -ScriptPath $ScriptPathOOBE
Create-SendKeysScript -ScriptPath $ScriptPathSendKeys

$ServiceUIPath = "C:\OSDCloud\ServiceUI.exe"
Download-ServiceUI -DownloadPath $ServiceUIPath
Validate-FileExists -Path $ServiceUIPath -ErrorMessage "ServiceUI.exe download failed or not found."

$SendKeysTaskName = "Scheduled Task for SendKeys"
$OSDCloudTaskName = "Scheduled Task for OSDCloud post installation"

$SendKeysTaskParams = @{
    TaskName        = $SendKeysTaskName
    TaskDescription = "Task to run SendKeys script"
    ExecutablePath  = $ServiceUIPath
    Arguments       = "-process:RuntimeBroker.exe C:\WINDOWS\System32\WindowsPowerShell\v1.0\powershell.exe $ScriptPathSendKeys -NoExit"
    TriggerDelay    = 'PT15S'
}

$OSDCloudTaskParams = @{
    TaskName        = $OSDCloudTaskName
    TaskDescription = "Task to run OSDCloud post installation script"
    ExecutablePath  = $ServiceUIPath
    Arguments       = "-process:RuntimeBroker.exe C:\WINDOWS\System32\WindowsPowerShell\v1.0\powershell.exe $ScriptPathOOBE -NoExit"
    TriggerDelay    = 'PT20S'
}

Create-ScheduledTask @SendKeysTaskParams
Create-ScheduledTask @OSDCloudTaskParams
